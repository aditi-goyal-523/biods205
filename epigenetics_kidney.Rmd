...
control_1w <-results(dds, contrast=c("group","1w","control"), tidy = TRUE) 
summary(control_1w)
head(control_1w[order(control_1w$padj),], 10)


# Control vs. 2 week
control_2w <- results(dds, contrast=c("group", "2w", "control"), tidy = TRUE)
summary(control_2w)
head(control_2w[order(control_2w$padj),], 10)

# Control vs. 4 week
control_4w <- results(dds, contrast=c("group", "4w", "control"), tidy = TRUE)
summary(control_4w)
head(control_4w[order(control_4w$padj),], 10)

# Control vs. 8 week
control_8w <- results(dds, contrast=c("group", "8w", "control"),tidy = TRUE)
summary(control_8w)
head(control_8w[order(control_8w$padj),], 10)
...
# male results
male1wk<-topTable(limma_res, coef = "control_1wk_M", n=Inf)
male1wk %>% 
  filter(adj.P.Val < 0.05)
male2wk<-topTable(limma_res, coef = "control_2wk_M", n=Inf)
male2wk %>% 
  filter(adj.P.Val < 0.05)
male4wk<-topTable(limma_res, coef = "control_4wk_M", n=Inf)
male4wk %>% 
  filter(adj.P.Val < 0.05)
male8wk<-topTable(limma_res, coef = "control_8wk_M", n=Inf)
male8wk %>% 
...
# female results
female1wk<-topTable(limma_res, coef = "control_1wk_F", n=Inf)
female1wk %>% 
  filter(adj.P.Val < 0.05)
female2wk<-topTable(limma_res, coef = "control_2wk_F", n=Inf)
female2wk %>% 
  filter(adj.P.Val < 0.05)
female4wk<-topTable(limma_res, coef = "control_4wk_F", n=Inf)
female4wk %>% 
  filter(adj.P.Val < 0.05)
female8wk<-topTable(limma_res, coef = "control_8wk_F", n=Inf)
female8wk %>% 
...
# Histogram
female2wk$pbins<-NULL
female2wk$pbins[female2wk$adj.P.Val > 0 & female2wk$adj.P.Val < 0.05] <- "<0.05"
female2wk$pbins[female2wk$adj.P.Val >=0.05 & female2wk$adj.P.Val < 0.3] <- "0.05-0.3"
female2wk$pbins[female2wk$adj.P.Val >=0.30 & female2wk$adj.P.Val < 0.6] <- "0.3-0.6"
female2wk$pbins[female2wk$adj.P.Val >=0.60 & female2wk$adj.P.Val < 0.9] <- "0.6-.9"
female2wk$pbins[female2wk$adj.P.Val >=0.90 & female2wk$adj.P.Val < 1] <- "0.9-1.0"

female2wk<-female2wk %>% 
  filter(is.na(adj.P.Val) == FALSE)

ggplot(data = female2wk, 
       aes(x = pbins)) + 
  geom_histogram(stat = "count", color="lightpink", fill="lightpink") +
  stat_count(binwidth = 1, 
             geom = 'text', 
  ...
  scale_fill_discrete(drop=FALSE) +
          scale_x_discrete(drop=FALSE)+
  ggtitle("Limma: Adjusted p-value distribution (female control vs. female 2 wk)")+
  theme(plot.title = element_text(hjust = 0.5))+
  labs(x="Adjusted p-value bins", y="Number peaks")
...
sig <- fgsea_results %>% filter(padj < 0.05)
w8_sig <- sig$pathway
```
...
print(fgsea_results2$pathway[0:20])
```
...
Make a venn diagram of how many significant genesets overlap between the time points

```{r}
library(ggvenn)
sig_gene_sets <- list('1w'=w1_sig,'2w'=w2_sig,'4w'=w4_sig, '8w'=w8_sig)
ggvenn(sig_gene_sets, show_percentage = FALSE, text_size = 6, label_sep = "\t")
```
