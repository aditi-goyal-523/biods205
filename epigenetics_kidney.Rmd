...
setwd("/Users/aubreyroberts/Documents/GitHub/biods205_proj/")
...
ATAC_FEATURE_ANNOT %>% 
  filter(feature_ID %in% rownames(filtered_raw_counts))

# Peaks they said did DA for
ATAC_KIDNEY_DA

# Peaks they said were DA that are promoters
test<-ATAC_KIDNEY_DA %>% 
  filter(feature_ID %in% rownames(filtered_raw_counts))

length(unique(test$feature_ID))

#  Peaks they said were DE that are promoters that are adjusted < 0.05
test$qvalue <- 
...
female<-kidney_PHENO2 %>% 
  filter(sex == "female")
femaleIDs<-rownames(female)

male<-kidney_PHENO2 %>% 
  filter(sex == "male")
maleIDs<-rownames(male)

# Subset filtered_raw_counts to female and male
female_filtered_raw_counts<-filtered_raw_counts[colnames(filtered_raw_counts) %in% femaleIDs]
male_filtered_raw_counts<-filtered_raw_counts[colnames(filtered_raw_counts) %in% maleIDs]
```
# DESeq2 for differential expression of promoter peaks (Both Sexes)
...
# DESeq2 for female and male separately
*Note: 1 significant upregulated peak in female at 2 weeks: chrY:554247-554590; none for males.*
```{r separate}

# Female - DESeq2
dds <- DESeqDataSetFromMatrix(countData = female_filtered_raw_counts, 
                              colData = female,
                              design = ~Sample_batch+peak_enrich.frac_reads_in_peaks.macs2.frip+group)
...
```{r male}
# Male - DESeq2
dds <- DESeqDataSetFromMatrix(countData = male_filtered_raw_counts, 
                              colData = male,
                              design = ~Sample_batch+peak_enrich.frac_reads_in_peaks.macs2.frip+group)
...
2) use model with covariates (sex, sample_batch, and peak_enrich.frac_reads_in_peaks.macs2.frip, and/or sex*group interaction)
No significant findings for male and female together for promoter peaks.
```{r limmarepro}
...
# Limma for female alone
```{r limma_sex}

# Convert to factor
female$Sample_batch<-as.factor(female$Sample_batch)

# Create design matrix with model.matrix() -- specifies which variables/coefficients will be in linear model
full_model_str = "~ 1 +  group + Sample_batch + peak_enrich.frac_reads_in_peaks.macs2.frip"
design = model.matrix(as.formula(full_model_str), data=female)
design

# Limma model
v = voom(female_filtered_raw_counts, design, normalize.method="quantile")
limma_model1 = lmFit(v, design)

# contrasts from limma_fit1: "group2w"   "group4w"    "group8w"  "groupcontrol"
...
# Limma for male alone
```{r limma_sex}

# Convert to factor
male$Sample_batch<-as.factor(male$Sample_batch)

# Create design matrix with model.matrix() -- specifies which variables/coefficients will be in linear model
full_model_str = "~ 1 +  group + Sample_batch + peak_enrich.frac_reads_in_peaks.macs2.frip"
design = model.matrix(as.formula(full_model_str), data=male)
design

# Limma model
v = voom(male_filtered_raw_counts, design, normalize.method="quantile")
limma_model1 = lmFit(v, design)
...
for (i in seq(length(allgenes))){
  ...
  wk4 <- as.numeric(filtered_norm_counts[i,week4_ids])
...
# GSEA - See Sophie's code instead. Just notes below.
...
# Limma on all peaks, not just promoter
```{r}

# Use all normalized data (not filtered for promoter peaks): NORM_DATA_full

ATAC_KIDNEY_RAW_COUNTS<-ATAC_KIDNEY_RAW_COUNTS %>% 
  select(-c(1,3:4))
rownames(ATAC_KIDNEY_RAW_COUNTS)<-ATAC_KIDNEY_RAW_COUNTS$feature_ID
ATAC_KIDNEY_RAW_COUNTS$feature_ID<-NULL

# Use Phenotype data from kidney_PHENO2
...
# Limma model
v = voom(ATAC_KIDNEY_RAW_COUNTS, design, normalize.method="quantile")
limma_model1 = lmFit(v, design)
